    ...
    while(S--):
        ...
        while t < T:
            ...
            while not Q.empty():
                ...
                neighOfCurr = list(G.neighbors(curr))
                random.shuffle(neighOfCurr)
                neighToBeAffected = int(len(neighOfCurr) * beta) + 1
                count = 0
                idx = 0
                while count < neighToBeAffected and idx < len(neighOfCurr):
                    if neighOfCurr[idx] not in recovered and neighOfCurr[idx] not in infected:
                        infected.append(neighOfCurr[idx])
                        count += 1
                    idx += 1
            for i in infected:
                Q.put(i)
            ft[t] = ft[t] + (len(recovered)+len(Q))/nodeCount
            infected.clear()
            t += 1
    ft[:]=[x/S for x in ft]
    return ft
